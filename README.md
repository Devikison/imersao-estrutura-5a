...
2. **Faixa do 1º lote** (`.lote`) — logo depois do hero, fundo escuro. O cronômetro dela usa `.cdown-grid.dourado`: plaquinhas douradas com o número preto (o do CTA final continua escuro com número claro). "Encerra em" e os rótulos das plaquinhas ficam em branco só aqui. Selo "1º lote" acima de "Encerra em", na mesma coluna do cronômetro; à esquerda, "36% das vagas já foram preenchidas" e a barra de progresso (a porcentagem vem da variável CSS `--vagas` no `style` da própria seção, e a barra enche quando entra na tela) e o cronômetro em versão compacta à direita. No mobile o cronômetro vai para cima e o resto centraliza.
...
- **Checkout / popup de captura:** os 5 botões de CTA (hero, esteira, card da oferta, CTA final e barra fixa) têm `data-cta` e abrem o **popup** (`#lead`), que envia nome, e-mail, WhatsApp, a origem do clique, a URL da página e as UTMs para o webhook do n8n por POST. O `href="#inscricao"` continua como plano B se o JavaScript não rodar. **Falta:** o fluxo do n8n precisa (a) aceitar `application/x-www-form-urlencoded` com esses nomes de campo e (b) responder com a URL do checkout (redirect, texto puro ou JSON com `url`/`checkout`/`link`/`redirect`). Se ele não devolver nada, preencha a constante `CHECKOUT` no topo do `<script>`; enquanto as duas estiverem vazias o popup mostra a tela de confirmação e ninguém chega ao pagamento. Também vale conferir o CORS no nó Webhook: sem `Access-Control-Allow-Origin`, a página entrega os dados mas não consegue ler a resposta (o popup cai na tela de confirmação).

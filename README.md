...
3. **Faixa do 1º lote** (`.lote.ouro`) — logo depois da faixa dourada, em dourado um tom abaixo dela (`#E3AC0B` a `#D19A06`) para as duas não virarem um bloco só; plaquinhas e barra escuras por cima. A classe `ouro` carrega toda a inversão de cores: tirar ela devolve a faixa ao fundo escuro. Selo "1º lote", "36% das vagas já foram preenchidas", barra de progresso (a porcentagem vem da variável CSS `--vagas` no `style` da própria seção, e a barra enche quando entra na tela) e o cronômetro em versão compacta à direita. No mobile o cronômetro vai para cima e o resto centraliza.

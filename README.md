...
- **Site no ar:** https://grupo2h.com.br/imersao5a — servido pelo GitHub Pages a partir do repo `centrallioxdigital-commits/grupo2h-formularios`, na pasta `imersao5a/`, ao lado de `/ficha` e `/diagnostico`. **Este repositório é a fonte de desenvolvimento e não publica em domínio próprio.** O Pages aceita um domínio personalizado por repositório, e `grupo2h.com.br` pertence ao repo dos formulários — por isso a LP mora lá como subpasta, em vez de ter um domínio apontado para cá. Domínios já aposentados, nesta ordem: `imersao5a.sitepreviavisual.site` e depois o subdomínio da antiga marca da agência, desativado em set/2026 junto com a marca.
- **Para publicar uma alteração:** edite aqui, monte a pasta com os arquivos que o `index.html` realmente referencia — são 20, o `index.html` e 19 assets; os PNG-fonte do hero e os SVG não usados ficam de fora — e suba essa pasta como `imersao5a/` no repo dos formulários.
...
2. **Faixa do 1º lote** (`.lote`) — logo depois do hero, fundo escuro. O cronômetro dela usa `.cdown-grid.dourado`, igual ao do CTA final: plaquinhas douradas, número preto e rótulos brancos. "Encerra em" também fica branco. Selo "1º lote" à esquerda no desktop (`.lote-tag-d`) e acima de "Encerra em" no mobile (`.lote-tag-m`) — são dois elementos, e cada largura mostra um. A coluna da esquerda traz "73% das vagas já foram preenchidas", a barra de progresso (a porcentagem vem da variável CSS `--vagas` no `style` da própria seção, e a barra enche quando entra na tela) e, abaixo dela, a linha de data — `.lote-data`, ícone de calendário + "10 de outubro, às 19:00 · ao vivo e online", que antes era uma pílula no hero. A coluna da direita traz o cronômetro em versão compacta. No mobile o cronômetro vai para cima, o resto centraliza e o ícone de calendário passa a fluir junto com o texto.
...
9. **A oferta** — título "Tudo o que a imersão entrega por R$ 27" sempre em duas linhas. Não é um `<br>`: cada metade é um `<span>` com `display:block` e `white-space:nowrap` (`.oferta-h2 span`), então o navegador não consegue partir nenhuma das duas, e o cabeçalho usa `.oferta-head{max-width:min(920px,100%)}` para ter folga. O `<br>` sozinho não bastava — quando a fonte ainda não tinha carregado, o texto na fonte substituta precisava de 649px em vez de 536 e a primeira linha quebrava no meio. Abaixo de 360px o `nowrap` é liberado, para não estourar a tela. Card escuro com borda animada, logo da imersão (`.offer-logo`) no topo à esquerda, ao lado do selo "À venda hoje", preço R$ 27, checklist, botão e a nota "Pagamento seguro" abaixo dele.
...
12. **Inscrição** — CTA final com anéis e brilho. Entre os chips e o botão fica o **cronômetro** (`.cdown.cdown-grid.dourado`, split-flap: dias, horas, minutos e segundos, no mesmo dourado da faixa do 1º lote) com o título "As inscrições encerram em breve.".
...
- **Checkout / popup de captura:** os 5 botões de CTA (hero, esteira, card da oferta, CTA final e barra fixa) têm `data-cta` e abrem o **popup** (`#lead`), que envia nome, e-mail, WhatsApp, a origem do clique, a URL da página e o rastreio para o webhook do n8n por POST e, em seguida, manda a pessoa para o checkout. O **rastreio** (`utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`, `utm_id`, `src`, `gclid`, `fbclid`) é lido da URL na chegada e guardado em `sessionStorage` (`rastreio5a`), então sobrevive a um F5 ou a uma volta pelo histórico; vai junto tanto para o webhook quanto para o checkout. Se a pessoa entrou sem nenhum parâmetro na URL, não há rastreio para enviar — é o caso de quem digita o endereço direto. O `href="#inscricao"` continua como plano B se o JavaScript não rodar. O destino é a constante `CHECKOUT` no topo do `<script>` (hoje `https://pay.kiwify.com.br/u3dyAhj`); se o fluxo do n8n responder com uma URL própria (redirect, texto puro ou JSON com `url`/`checkout`/`link`/`redirect`), ela ganha. Nos dois casos a página acrescenta `customer.name`, `customer.email` e `customer.phone` (com o 55 na frente) para o checkout abrir preenchido — são os parâmetros que a Kiwify documenta. **Falta confirmar com o n8n:** se o fluxo aceita `application/x-www-form-urlencoded` com os nomes `nome`, `email` e `whatsapp`, e se o nó Webhook libera o CORS para `https://grupo2h.com.br` (sem isso os dados chegam, mas a página não consegue ler a resposta — o redirecionamento continua funcionando pela constante).
...
- **Seta dourada do hero:** a seta neon inline só funciona sobre área escura, então a posição dela depende da capa em uso e precisa ser medida a cada troca de arte. O método: luminância média do PNG-fonte na área que a seta ocupa, de 0 a 255; abaixo de ~60 ela tem contraste. Histórico: na capa do suéter cru a posição original do designer media 209 no desktop e 198 no mobile, e as duas tiveram de sair de lá. Na capa do suéter azul-marinho a original voltou a servir no desktop (27), e por isso o desktop foi devolvido para `top:85.2%;right:45%`. No mobile a original ficou em 63, limítrofe, contra 11 em `top:68%;right:12%` — então o mobile segue deslocado. Ali a seta usa `right`, e por isso a animação tem de ser `float-bob`, que é a do `translate(50%,-50%)`; com `float-bob-l` o elemento sai meia largura fora do lugar.
- Capas do hero: desktop 1920×1080 com a pessoa à direita, sobre o corte diagonal amarelo; mobile 1080×1620 com a pessoa centralizada. Nenhuma das duas traz o letreiro embutido — o único letreiro da página é o `<img class="hero-logo">`. Se a arte mudar de composição, revisar a posição dos ícones flutuantes e o `margin-top` negativo do texto no mobile (`.hero-content-wrap` na media query de 900 px).
- **Tamanho da logo do hero, por faixa:** ate 760px (celular, texto centralizado) `min(88%,325px)`; de 761 a 900px (tablet em retrato, mesmo layout vertical, texto à esquerda) `clamp(300px,42vw,325px)`; de 901px para cima (desktop) `clamp(300px,27vw,400px)`. As três andam juntas de propósito: a logo cresce de ~285px num celular estreito até 400px numa tela grande, sem degrau visível nas trocas de faixa. Mexer em uma só reabre o degrau — já houve um de 115px entre 760 e 761px. O arquivo tem 619px de largura nativa, então até 400px a logo ainda é reduzida e não borra.
- **Bloco de texto do hero no desktop (`@media (min-width:901px)`):** a coluna `.hero-text-col` é centralizada na seção, então crescer a coluna sobe o topo dela metade do que cresceu e desce a base a outra metade. É assim que logo, título e subtexto sobem com os botões parados: a `.hero-cta-row` ganha `--hero-lift` de margem e a coluna volta metade disso no `translateY`. O `--logo-extra` desconta a altura que a logo maior acrescenta, senão os botões desceriam ~10px. Mexer em um desses três valores sem os outros desalinha os botões. Abaixo de 901px nada disso se aplica e o mobile fica como sempre foi.

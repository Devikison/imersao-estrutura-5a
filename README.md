...
- **Link de pagamento:** os 5 botões de CTA (hero, esteira, card da oferta, CTA final e barra fixa) apontam para o webhook do n8n `https://n8n-n8n-start.dnjlb7.easypanel.host/webhook/grupo2h`, pedido pelo cliente. **Ainda não foi validado:** para o clique funcionar, o fluxo precisa estar ativo, o nó Webhook precisa aceitar GET e a resposta precisa ser um redirect (nó *Respond to Webhook* em modo Redirect) para o checkout — senão a pessoa cai numa tela de JSON. Testar abrindo o link no navegador antes de publicar. O link "Inscrição" do rodapé continua sendo âncora interna, de propósito.
- **Popup de captura:** o cliente falou em "popup no botão" — um modal com formulário (nome, e-mail, WhatsApp) que envia os dados ao webhook e só depois manda para o checkout. Não foi implementado: faltam os campos que o fluxo espera, o método (GET/POST) e o que ele devolve.

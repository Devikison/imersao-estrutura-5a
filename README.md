...
- **Seta dourada do hero (resolvido):** com as capas novas a seta neon inline caiu sobre o suéter cru e sumiu. Foi medida a luminância da arte na área que ela ocupa: 209 no desktop e 198 no mobile, de 255. Agora está em `top:85.2%;right:56%` no desktop (luminância 31, abaixo dos botões) e `top:68%;right:12%` no mobile (luminância 23, à direita da pessoa). No mobile ela trocou de `left` para `right`, então a animação virou `float-bob` — é a que usa `translate(50%,-50%)`; com `float-bob-l` o elemento sairia meia largura fora do lugar. Se a arte do hero mudar, medir de novo antes de reposicionar.
